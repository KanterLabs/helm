...
func TestAgentNotesMigrationPreservesPopulatedDatabaseAndRetainedWrites(t *testing.T) {
	...
	if len(migrations) < 24 {
		t.Skip("migration 024 is not present")
	}
	database := newRawDatabase(t)
	applyMigrationPrefix(t, ctx, database, migrations, 23)
	populateProductionFixture(t, ctx, database, 23)
	...
	if err := Migrate(ctx, database); err != nil {
		t.Fatalf("migrate populated pre-024 database: %v", err)
	}
	...
	// A retained pre-024 binary can continue mutating the old task shape.
	...
	if err := CheckIntegrity(ctx, database); err != nil {
		t.Fatalf("migration 024 integrity: %v", err)
	}
}
