#!/usr/bin/env python3
"""Codex lifecycle hooks for the Helm agent-work pulse.
...
def _context(state: helm_session.SessionState, *, compact: bool = False) -> str:
    ...
    return f"{prefix} Helm task {task}{project}: agent state {state.agent_state}{checkpoint}; operation {state.operation_id}{reminder}."


def _agent_notes_context(state: helm_session.SessionState) -> str:
...
def _interval_seconds(environ: Mapping[str, str] | None = None) -> float:
    ...
    for key in (
        ...
        if value >= 0:
            return value
        break
    return helm_session.HEARTBEAT_INTERVAL_SECONDS
...
def _heartbeat(store: helm_session.StateStore, *, now: datetime | None = None) -> helm_session.HeartbeatResult:
    ...
    def send(task_id: str, operation_id: str) -> Any:
        ...
        config = helm.load_config()
        client = helm.Client(config)
...
def handle_event(
    ...
    *,
    ...
    the Helm service is unavailable.
    """
    ...
    if name == "Stop" and _truthy(event.get("stop_hook_active")):
        return _safe_empty()
    try:
        store = (store_factory or helm_session.StateStore.from_event)(event)
    ...
    try:
        ...
        if should_block and state is not None:
            ...
            return {
                "decision": "block",
                "reason": f"Helm task {task} is still marked {state.agent_state}; complete it, block it, or publish structured waiting/handoff progress before stopping.",
            }
        return _safe_empty()
    except (helm_session.SessionStateError, helm.HelmError, OSError) as exc:
        _warning(exc)
        return _safe_empty()
