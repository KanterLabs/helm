...
def _collection_cursor(payload: Any) -> str:
    ...
    if not isinstance(cursor, str):
        raise RoadmapError("TC Roadmap returned an invalid collection cursor")
    return cursor.strip()
    ...
    *,
    ...
    while True:
        ...
        try:
            ...
            raise RoadmapError(f"invalid {context} collection response") from exc
        if not next_cursor:
            return result
        if next_cursor in seen or next_cursor == cursor:
            raise RoadmapError(f"{context} collection returned a repeated cursor")
...
def _paged_collection_with_cursor(
    ...
    *,
    ...
    """Read one or every cursor page while preserving the terminal cursor."""
...
def _audit_states(value: Any) -> tuple[str, ...]:
    ...
    if isinstance(value, str):
        ...
        raise RoadmapError("audit states must be semantic state names")
    states: list[str] = []
    for item in values:
        ...
        if state not in states:
            states.append(state)
    if not states:
        raise RoadmapError("audit requires at least one semantic state")
    return tuple(states)
...
def _audit_page_size(value: Any) -> int:
    try:
        ...
        raise RoadmapError("audit page size must be between 1 and 200") from exc
    if parsed < 1 or parsed > 200:
        raise RoadmapError("audit page size must be between 1 and 200")
    return parsed
...
def _audit_evidence_limit(value: Any) -> int:
    try:
        ...
        raise RoadmapError("audit evidence limit must be between 0 and 5") from exc
    if parsed < 0:
        raise RoadmapError("audit evidence limit must be between 0 and 5")
...
def _audit_error_context(exc: RoadmapError) -> dict[str, Any]:
...
def _audit_submission_items(raw: Any) -> list[Any]:
    ...
    if isinstance(value, (str, Path)):
        if str(value) == "-":
            ...
            try:
                ...
                raise RoadmapError("cannot read audit submission input") from exc
        try:
            value = json.loads(encoded)
        except json.JSONDecodeError as exc:
            raise RoadmapError("audit submission input is not valid JSON") from exc
    if isinstance(value, dict):
    ...
    if not isinstance(value, list):
        raise RoadmapError("audit submission input must contain a findings list")
    return value
...
def _audit_run_id(payload: Any) -> str:
    if not isinstance(payload, dict):
        raise RoadmapError("TC Roadmap returned an unexpected audit run")
...
def _release_project(client: Client, reference: str) -> dict[str, Any]:
    ...
    if not normalized:
        raise RoadmapError("project must not be empty")
    ...
    if not matches:
        raise RoadmapError(f"project not found: {normalized}")
    if len(matches) > 1:
        raise RoadmapError(f"project reference is ambiguous: {normalized}")
    project = matches[0]
    if not isinstance(project.get("id"), str) or not project["id"].strip():
        raise RoadmapError("TC Roadmap returned an unexpected project")
    return project
    ...
    *,
    ...
    if status:
        ...
        if normalized_status not in {"planned", "released"}:
            raise RoadmapError("status must be planned or released")
...
def _release_reference(
    ...
    if not normalized:
        raise RoadmapError("release must not be empty")
    ...
    if not matches:
        raise RoadmapError(f"release not found: {normalized}")
    if len(matches) > 1:
        raise RoadmapError(f"release reference is ambiguous: {normalized}")
    release = matches[0]
    if not isinstance(release.get("id"), str) or not release["id"].strip():
        raise RoadmapError("TC Roadmap returned an unexpected release")
...
def cmd_submit(client: Client, args: argparse.Namespace) -> Any:
    ...
    if not scope:
        raise RoadmapError("audit submission scope is required")
    ...
    if requested_audit:
        ...
        if _audit_safe_identifier(run.get("project_id")) != project_id:
            raise RoadmapError("queued audit belongs to a different project")
        if run.get("status") not in {"queued", "running"}:
            raise RoadmapError("audit run is already finalized")
    ...
    if requested_status not in {"complete", "partial", "failed"}:
        raise RoadmapError("audit submission status must be complete, partial, or failed")
...
def cmd_reconcile(client: Client, args: argparse.Namespace) -> Any:
    ...
    if not audit_id:
        raise RoadmapError("audit ID is required")
    ...
    if not project_id:
        raise RoadmapError("audit run has no project")
    ...
    for finding in findings:
        ...
        try:
            ...
            if not isinstance(current, dict) or not isinstance(current.get("version"), int):
                raise RoadmapError("unexpected task")
        except RoadmapError as exc:
...
def _validate_limit(value: Any, *, default: int = 50) -> int:
    ...
    try:
        ...
        raise RoadmapError("limit must be an integer between 1 and 200") from exc
    if parsed < 1 or parsed > 200:
        raise RoadmapError("limit must be an integer between 1 and 200")
    return parsed
...
def cmd_releases_list(client: Client, args: argparse.Namespace) -> Any:
    ...
    return {"project": project.get("key"), "releases": releases, "next_cursor": next_cursor}


def cmd_releases_get(client: Client, args: argparse.Namespace) -> Any:
    """Resolve a project-local release name/ID, then read its resource."""
    ...
    if isinstance(payload_project, str) and payload_project.strip() != str(project["id"]).strip():
        raise RoadmapError("release does not belong to the requested project")
    return payload
...
def _release_queue_page(
    ...
    *,
    ...
    if not isinstance(payload, dict):
        raise RoadmapError("TC Roadmap returned an unexpected release work queue")
    ...
    try:
        ...
        raise RoadmapError("invalid release work queue response") from exc
...
def _release_workflow_projection(queue: dict[str, Any]) -> dict[str, Any]:
    ...
    direct, required, completed = count("direct"), count("required"), count("completed")
    owned, claimable = count("owned"), count("claimable")
    blockers = (
    ...
    blocked = [(label, count(name)) for name, label in blockers if count(name) > 0]
    if direct > 0 and required > 0 and completed >= required:
        state, next_action = "ready", "release_review"
        guidance = [
        ...
        state, next_action = "handoff", "resume_owned_task"
        guidance = [
        ...
        state, next_action = "handoff", "claim_one_task"
        guidance = [
        ...
        state, next_action = "waiting", "confirm_release_scope"
        guidance = [
        ...
        state, next_action = "waiting", "resolve_blockers"
        guidance = [
    ...
    if blocked:
        guidance.append("Current queue blockers: " + ", ".join(f"{label} ({amount})" for label, amount in blocked) + ".")
        ...
        try:
            while True:
                ...
                if not collect_all or not next_cursor:
                    ...
                    return result
                if next_cursor in seen or next_cursor == cursor:
                    raise RoadmapError("release work queue returned a repeated cursor")
                seen.add(next_cursor)
                cursor = next_cursor
        except RoadmapError as exc:
...
# Alternate function names make the compatibility module's public dispatch
# surface match the canonical Helm helper.
...
def cmd_release(client: Client, args: argparse.Namespace) -> Any:
    """Release (unclaim) the current owner's active task lease."""

    supplied = getattr(args, "operation_id", None)
    operation_id = str(supplied).strip() if supplied is not None else ""
    if not operation_id:
        operation_id = str(uuid.uuid4())
    else:
        operation_id = _validate_operation_id(operation_id)
    current = _task(client, str(getattr(args, "task", "")))
    path = "/tasks/" + parse.quote(str(current["id"]), safe="") + "/release"
    payload, _ = client.call(
        "POST",
        path,
        if_match=current["version"],
        idempotency_key=_mutation_idempotency(operation_id, "POST", path),
    )
    _clear_matching_session(current, operation_id)
